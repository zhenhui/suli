...
define("sjplus/global/0.0.1/crypto/sha3-debug", [], function(require, exports, module) {
    var CryptoJS = CryptoJS || function(v, p) {
        var d = {}, u = d.lib = {}, r = function() {}, f = u.Base = {
            ...
            parse: function(a) {
                for (var b = a.length, c = [], j = 0; j < b; j += 2) c[j >>> 3] |= parseInt(a.substr(j, 2), 16) << 24 - 4 * (j % 8);
        ...
        d = d.SHA3 = r.extend({
            ...
            _doProcessBlock: function(a, b) {
                ...
                for (f = 0; 24 > f; f++) {
                    ...
                    for (m = 1; 25 > m; m++) g = e[m], h = g.high, g = g.low, k = s[m], 32 > k ? (l = h << k | g >>> 32 - k, 
                    d = g << k | h >>> 32 - k) : (l = g << k - 32 | h >>> 64 - k, d = h << k - 32 | g >>> 64 - k), 
                    g = c[x[m]], g.high = l, g.low = d;
                    g = c[0];
                    h = e[0];
                    g.high = h.high;
                    g.low = h.low;
                    for (h = 0; 5 > h; h++) for (k = 0; 5 > k; k++) m = h + 5 * k, g = e[m], l = c[m], 
                    m = c[(h + 1) % 5 + 5 * k], d = c[(h + 2) % 5 + 5 * k], g.high = l.high ^ ~m.high & d.high, 
