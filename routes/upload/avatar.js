...
exports.saveFile = function (req, res) {
        ...
        gm(file.path).format(function (err, format) {
            ...
            if (!err && allowFile[format.toLowerCase()]) {
                ...
                gm(file.path).size(function (err, size) {
                    if (!err && size.width === 80 && size.height === 80) {
    ...
    function resize(file, ownerID) {
        ...
        function _resize() {
            ...
            //生成缩略图并存入库中
            var fileName = ownerID + '_' + file.fileId + '_' + cur.width + 'x' + cur.height
            ...
            switch (file.format) {
                ...
                case 'jpg':
                    gm(file.path).resize(cur.width, cur.height).noProfile().quality(cur.quality).write(dstSrc, function (err) {
                        if (!err) {
...
app.post('/upload/avatar', exports.saveFile)
